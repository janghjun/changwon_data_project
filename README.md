...
### 분석 데이터
	1.	정류소/노선 데이터
	•	/master/stops_master.csv
- 창원시 전체 버스정류소 기본 정보 (정류소 ID, 위도·경도, 행정동 코드, 설치년도)
	•	/master/stops_snapshots.csv
- 연도별 정류소 위치 스냅샷 (2020, 2022, 2023, 2024년)
	•	/master/stops_counts_by_emd_year.csv
- 읍·면·동 단위 연도별 정류소 수 집계
	2.	청년 인구 데이터
	•	/master/population_youth_master_with_adm7.csv
- 읍·면·동별 전체 인구(pop_total), 청년 인구(20–34세), 청년 비중(youth_share), 연도 정보 포함
	3.	정제 결과 지표
	•	/master/b1_accessibility.csv
- 정류소 접근성 지표
	•	stops_count: 절대 정류소 수
	•	stops_per_km2: 면적당 정류소 수 (밀도)
	•	coverage_pct: 정류소 400m 버퍼 커버 면적 비율
	•	stops_per_10k_youth: 청년 인구 1만 명당 정류소 수
	•	/master/b2_routes.csv
- 노선 다양성 지표
	•	routes_unique: 고유 노선 수
	•	routes_per_stop: 정류소당 평균 노선 수
	•	route_diversity: 노선 다양성 지수
...
### 주요 결과
	•	강한 양(+) 상관
	•	정류소 밀도 ↔ 커버리지 (r≈0.88)
	•	노선 관련 지표(고유 노선 수, 정류소당 노선 수, 다양성) 간 상관 (r≈0.87~0.93)
- 도심부일수록 정류소 접근성과 노선 다양성이 함께 강화
	•	음(-) 상관
	•	정류소 절대수 ↔ 품질지표(밀도·커버리지·다양성) (r≈-0.30~-0.48)
- 단순 개수(stops_count)는 면적효과로 왜곡 → 보조지표로만 활용
	•	중간 수준 양(+) 상관
	•	정류소 밀도·커버리지 ↔ 노선 다양성·고유 노선 수 (r≈0.15~0.28)
- 밀도가 높은 지역일수록 교통 서비스 선택지가 조금 더 풍부

⸻

### 스토리라인
	1.	교통 서비스 품질(밀도·커버리지·노선 다양성)은 도심에서 함께 강화됨
	2.	절대 정류소 수(stops_count)는 해석에 왜곡 가능성 → 보조 변수로 활용
	3.	청년 인구 연결은 2020~2024년 패널 데이터 보강 필요

⸻

### 결론: 향후 B1~B5 상관분석에서는 정규화된 품질 지표 중심으로 분석해야 함.

⸻
# 09/11 진행 개선

## 소주제_교통 (형준 담당) – 상관분석 정리 (수정본)

### B1. 버스 배차간격(빈도) ↔ 청년 밀집도
	•	질문: 버스 배차간격이 촘촘할수록(=운행 빈도↑) 정류장 주변 청년 밀집도가 높을까?
	•	데이터/지표:
	•	버스 도착정보(API / arrivals_raw, headway_station_route_bucketed.csv)
	•	정류소 위치 (stops_master.csv, stops_snapshots.csv)
	•	청년 인구 (population_youth_master_with_adm7.csv)
	•	정제 지표:
	•	headway_mean_min, headway_std_min, headway_cv_mean
	•	400m 버퍼 내 youth_share
	•	분석: EMD 또는 정류소 버퍼 단위로 빈도 vs 청년비중 상관/회귀.
	•	현황: 이미 B1 접근성/빈도 관련 b1_accessibility.csv 정리 → 청년 데이터만 보강하면 바로 적용 가능.

⸻

### B2. 정시성(헤드웨이 CV) ↔ ㎡당 전월세 단가
	•	질문: 버스가 정시에 올수록 지역 임대료가 더 높아질까?
	•	데이터/지표:
	•	버스 도착 시계열 (headway_station_route_bucketed.csv) → headway_cv_mean
	•	국토부 실거래가 API (국토교통부_아파트 전월세 실거래가 자료)
	•	정제 지표:
	•	CV = 표준편차/평균
	•	price_per_m2 (임대차 거래 단가)
	•	분석: 공간조인(정류소 버퍼 ↔ 거래 좌표) 후 회귀분석.
	•	현황: 교통 쪽(B3·B4) 정제 완료, 부동산 데이터 API 연동만 추가 필요.

⸻

### B3. 정류장/노선 밀도 ↔ 소형주택 재고 비중
	•	질문: 대중교통 밀도가 높을수록 소형주택(청년 선호형)이 많이 분포하는가?
	•	데이터/지표:
	•	교통: stops_counts_by_emd_year.csv, b2_routes.csv (밀도/노선 수)
	•	주택: 건축물대장(WFS API) or 국토부 건축물대장 → floor_area, housing_type
	•	정제 지표:
	•	정류소·노선 밀도 (㎢당)
	•	소형주택 비중 = 전유면적 ≤ 60㎡ 주택 수 / 전체 주택 수
	•	분석: 읍면동 단위 상관.
	•	현황: 교통 데이터는 준비 완료, 소형주택 재고 데이터 추가 필요.

⸻

### B4. 교통혼잡(도로 평균속도) ↔ 청년 주거 선호
	•	질문: 혼잡이 심한 곳일수록 청년 선호가 낮아지는가?
	•	데이터/지표:
	•	교통: 국토부/도로공사 교통소통정보 API (평균속도, 지체지수)
	•	인구: population_youth_master_with_adm7.csv
	•	정제 지표:
	•	혼잡지수 = 기준속도 대비 실제속도
	•	청년 비중 youth_share
	•	현황: 혼잡데이터는 아직 없음 → 추가 수집 필요.

⸻

### B5. 환승거점 접근성 ↔ 임대료/입주율
	•	질문: 환승 접근성이 좋을수록 임대료·입주율이 높을까?
	•	데이터/지표:
	•	환승거점: stops_master.csv + 주요 환승터미널 좌표
	•	부동산: 전월세 실거래가 API
	•	공공임대: 마이홈포털 API (공공임대 단지정보/대기현황)
	•	정제 지표:
	•	접근성 지수 = 환승거점까지 최소 이동시간 (도보망 + 버스)
	•	임대료, 공공임대 입주율
	•	현황: 교통데이터+환승터미널 위치 가능, 임대료/입주율 데이터 보강 필요.

⸻

### B6. 산업단지 접근성 ↔ 청년 거주율
	•	질문: 산업단지 접근성이 좋을수록 청년 거주율이 높은가?
	•	데이터/지표:
	•	산단 위치: 국토부 산업단지 SHP
	•	교통: 버스노선 네트워크(routes_std.csv, stops_std.csv)
	•	인구: population_youth_master_with_adm7.csv
	•	정제 지표:
	•	버스 네트워크 기반 산단 접근성(통근 가능 시간)
	•	youth_share
	•	현황: 교통/인구 데이터 준비 완료, 산업단지 위치 데이터 필요.

⸻

## 전체 정리
	•	B1~B2: 이미 정제 완료한 b1_accessibility.csv, b2_routes.csv, b3/b4 punctuality 지표 + 청년 인구/실거래 API 연계 → 바로 분석 가능.
	•	B3~B6: 교통 데이터는 준비됨. 단, 주택(소형재고), 도로혼잡, 임대료, 공공임대, 산업단지 위치 데이터 추가 확보 필요.
	•	진행 순서:
	1.	B1 (배차빈도 ↔ 청년밀집도) — 데이터 바로 결합 가능 → 1차 실행.
	2.	B2 (정시성 ↔ 임대료) — 교통 데이터 OK, 실거래가 API 연동 추가.
	3.	B3 (밀도 ↔ 소형주택) — 소형주택 데이터 확보 필요.
	4.	B4 (혼잡 ↔ 청년선호) — 도로 혼잡 데이터 확보 필요.
	5.	B5 (환승거점 ↔ 임대료/입주율) — 임대료·임대주택 API 필요.
	6.	B6 (산단 접근성 ↔ 청년 거주율) — 산단 위치 데이터 필요.

⸻
# B1~B6 분석 계획 (B1부터 시작) 09/11

## 분석 단계
	1.	소주제 정의
	•	예: B1 = 버스 배차빈도(운행 간격) ↔ 청년 밀집도
	2.	데이터셋 식별 & 컬럼 파악 (📂 data head,value_counts.docx 기반)
	•	현재 우리가 가지고 있는 데이터 중 어떤 파일을 사용할 수 있는지 → 파일명, 경로, 컬럼명(영문/한글) 정리
	•	컬럼명은 한글로 해석 + 역할 정의
	•	예: station_id (정류소ID, 공간조인 키)
	•	예: headway_mean_s (평균 배차간격(초), 배차빈도 계산에 직접 활용)
	•	예: youth_share (해당 지역 청년 비중, 종속 변수 역할)
	3.	추가 데이터 필요 여부 판단
	•	없는 데이터는 웹서핑/크롤링으로 창원시 또는 공공데이터포털에서 확보 (예: 실거래가, 도로 혼잡, 소형주택 재고 등)
	•	확보한 데이터는 정제 후 master 폴더에 저장 → 상관분석에 통합
	4.	지표별 개별 분석/시각화
	•	예: B1의 경우
	•	(A) 배차빈도 지표 분석 (분포, 지역별 평균, 시각화)
	•	(B) 청년 밀집도 지표 분석 (연도별, EMD별 분포/지도 시각화)
	5.	상관분석 실행
	•	두 지표(배차빈도 vs 청년 밀집도) 상관계수 계산 (Spearman, Pearson 비교)
	•	유의성 검정(p-value), 데이터 수(N) 확인
	6.	상관분석 시각화
	•	산점도 + 회귀선
	•	히트맵 (변수들 간 상관 전체 보기)
	•	지도 시각화 (배차빈도/청년 밀집도의 공간적 분포 비교)
	7.	해석 및 스토리라인 작성
	•	“버스 배차빈도가 촘촘한 지역일수록 청년 인구 밀도가 높다/낮다”와 같은 구체적 해석 도출

⸻

## B1 (버스 배차빈도 ↔ 청년 밀집도) 구체 예시

### 1) 필요한 데이터
	•	배차빈도(Headway)
	•	 /master/b3_headway.csv
	•	station_id (정류소ID)
	•	year (연도)
	•	headway_mean_min (평균 배차간격, 단위: 분)
	•	headway_std_min (표준편차, 배차 안정성)
	•	headway_cv_mean (변동계수, 정시성 지표)
	•	stations_with_hw (해당 EMD 내 배차간격 산출된 정류소 수)
	•	역할: 배차빈도(독립 변수)로 사용
	•	청년 인구(밀집도)
	•	 /master/population_youth_master_with_adm7.csv
	•	adm_cd7 (법정동 코드, 7자리)
	•	year (연도)
	•	pop_total (전체 인구)
	•	youth_20_34 (20–34세 청년 인구 수)
	•	youth_share (청년 비중 = 청년 인구 / 전체 인구)
	•	역할: 청년 밀집도(종속 변수)로 사용
	•	공간 매핑용
	•	 /master/emd_boundary_master.geojson
	•	행정동 경계 → 시각화 및 공간조인용

### 2) 추가 데이터 필요 여부
	•	현재 보유 데이터만으로도 B1 상관분석 가능.
	•	다만, headway는 2025년 단일년도 중심이므로,
	•	필요시 2020~2024년 도착정보 API 호출/수집을 통해 패널 확장 가능.

### 3) 개별 분석
	•	(A) 배차빈도: headway_mean_min의 분포, 지역별 평균 지도
	•	(B) 청년 밀집도: youth_share의 지역별 분포 지도

### 4) 상관분석 실행
	•	조인: adm_cd7 + year 기준
	•	분석: headway_mean_min ↔ youth_share (Spearman, Pearson)
	•	시각화: 산점도 + 회귀선 / 연도별 비교
